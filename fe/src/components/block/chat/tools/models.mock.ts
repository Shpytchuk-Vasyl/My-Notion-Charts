...
const mockResponses: Record<string, string> = {
  default:
    "This is a mock response for testing. # 1. Отримуємо список усіх встановлених компонентів (назви файлів без розширення)",
...
function getResponseForPrompt(prompt: unknown[]): string {
  const promptStr = JSON.stringify(prompt[prompt.length - 1]).toLowerCase();
